...
EnrichIntegrationFromRemoteConfigStrategy.prototype._enrichWithDirectoryAccountModel = function (client, config, directory, callback) {
  ...
  directory.getAccountSchema(stopIfError(function (accountSchema) {
    accountSchema.getFields(stopIfError(function (fields) {
      var registrationFields = webConfig.register.form.fields;

      fields.each(function (field, next) {
        var fieldToConfigure = registrationFields[field.name];

        if (fieldToConfigure) {
          var localValue = registrationFields[field.name].required;
          var remoteValue = field.required;

          if (localValue === null) {
            fieldToConfigure.required = remoteValue;
          }
        ...
        // If a field is not configured, then don't require it.
        for (var key in registrationFields) {
          if (typeof registrationFields[key].required !== 'boolean') {
            registrationFields[key].required = false;
          }
        }
