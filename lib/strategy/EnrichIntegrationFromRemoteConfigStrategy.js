...
EnrichIntegrationFromRemoteConfigStrategy.prototype._validateAccountStore = function (config, app, callback) {
  app.getAccountStoreMappings(function (err, mappings) {
    ...
    if (mappings.size === 0) {
      return callback(new Error('No account stores are mapped to the specified application. Account stores are required for login and registration.'));
    } else if (config.web.register.enabled && app.defaultAccountStoreMapping === null) {
      return callback(new Error('No default account store is mapped to the specified application. A default account store is required for registration.'));
    }
