 ...
 * @class
*/
var EnrichIntegrationFromRemoteConfigStrategy = function (client) {
  this.client = client;
};
        ...
        if (/\/directories/.test(accountStore.href)) {
          accountStore.getProvider(function(err, remoteProvider) {
            ...
            if (['stormpath', 'ad', 'ldap'].indexOf(providerId) === -1) {
              ...
              extend(localProvider, remoteProvider, { enabled: true });
            }
            next();
          });
        } else {
...
EnrichIntegrationFromRemoteConfigStrategy.prototype._enrichWithDirectoryPolicies = function (config, directoryHref, callback) {
  if (!directoryHref) {
    return callback();
  }
  ...
  this.client.getDirectory(directoryHref, { expand: 'passwordPolicy,accountCreationPolicy' }, stopIfError(function (directory) {
...
EnrichIntegrationFromRemoteConfigStrategy.prototype.process = function (config, callback) {
  ...
  if (config.application) {
    tasks = tasks.concat([
      ...
      this._enrichWithDirectoryPolicies.bind(this, config)
    ]);
  }
